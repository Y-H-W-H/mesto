...
export class FormValidator {
    constructor(formSettings, formToValidate) {
        ...
        this._submitButton = formToValidate.querySelector(formSettings.submitBtnSelector);
        this._submitButtonDisabledSelector = formSettings.submitButtonDisabledSelector;
        ...
        this._formInputs = Array.from(formToValidate.querySelectorAll(this._inputSelector));
    }
    _toggleButtonState() {
        const hasInvalidInput = () => { return this._formInputs.some((inputElement) => !inputElement.validity.valid) };
        if (hasInvalidInput()) {
    ...
    _showErrorMessage(input) {
        const errorElement = this._formToValidate.querySelector(`.${input.id}-error`);
